...
class Main_model extends CI_Model
{
    ...
    public function view_join_one($table1, $table2, $field1, $field2, $order, $ordering)
    {
        ...
        $this->db->from($table1);
        $this->db->join($table2, $table1 . '.' . $field1 . '=' . $table2 . '.' . $field2);
        // $this->db->join($table2, 'ON '. $table1 . '.' . $field1 . '=' . $table2 . '.' . $field2);
        $this->db->order_by($order, $ordering);
        return $this->db->get();
    }

    public function view_join_where($table1, $table2, $field1, $field2, $field3, $where, $order, $ordering)
    {
        //join 1 table
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1 . '.' . $field1 . '=' . $table2 . '.' . $field2);
        $this->db->where($table1 . '.' . $field3 . '=' . $where);
        // $this->db->join($table2, 'ON '. $table1 . '.' . $field1 . '=' . $table2 . '.' . $field2);
        $this->db->order_by($order, $ordering);
        return $this->db->get();
    }
